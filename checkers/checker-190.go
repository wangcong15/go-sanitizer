...
func check190(c *code.Code) {
	lv := localVars190{
		funcName: "AssertOverflow",
		bugType:  "190",
	}
	lv.dirtyVals = make(map[string]int)
	x1List := Inspect(c.F, &ast.FuncDecl{})
	for _, x1 := range x1List {
		check190x1(c, x1.(*ast.FuncDecl), &lv)
	}
}

func check190x1(c *code.Code, x1 *ast.FuncDecl, lv *localVars190) {
	lv.x1 = x1
	...
	for _, x2 := range x2List {
		check190x2(c, x2.(*ast.ValueSpec), lv)
	}
	x4List := Inspect(x1, &ast.BinaryExpr{})
	for _, x4 := range x4List {
		check190x4(c, x4.(*ast.BinaryExpr), lv)
	}
}

func check190x2(c *code.Code, x2 *ast.ValueSpec, lv *localVars190) {
	lv.x2 = x2
	x3 := x2.Type
	if _, ok := x3.(*ast.Ident); ok {
		check190x3(c, x3.(*ast.Ident), lv)
	}
}

func check190x3(c *code.Code, x3 *ast.Ident, lv *localVars190) {
	lv.x3 = x3
...
func check190x4(c *code.Code, x4 *ast.BinaryExpr, lv *localVars190) {
	lv.x4 = x4
	if x4.Op.String() == "+" {
		exp1 := getExpr(x4.X)
		exp2 := getExpr(x4.Y)
...
func genAssert190(c *code.Code, lv *localVars190) {
